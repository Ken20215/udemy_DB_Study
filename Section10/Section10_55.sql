...
-- COUNT
SELECT COUNT(*) FROM customers; -- 何行データか
SELECT COUNT(name) FROM customers; -- 列指定（この行に何行入っているか(NULLはカウントしない)）

SELECT COUNT(name) FROM customers WHERE id>80;

-- MAX: 最大、MIN: 最小
SELECT MAX(age)c, MIN(age) FROM users WHERE birth_place="日本";
